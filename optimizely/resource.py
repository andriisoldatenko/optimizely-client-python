...
class ResourceGenerator(object):
    ...
    def get(self, optimizely_ids=None):
        ...
        elif isinstance(optimizely_ids, int):
            instance = self.resource(self.client, optimizely_id=optimizely_ids)
            instance.refresh()
            return instance
        elif isinstance(optimizely_ids, list):
            response_list = []
...
class APIObject(object):
    ...
    def refresh(self):
        if not hasattr(self, 'id'):
            raise AttributeError(
                '%s object has no ID, so it cannot be refreshed'.format(
                    self.class_name()
                )
            )
        self._refresh_from(self.client.request(
            'get', [self.class_url(), self.id]))
    ...
    def get_child_objects(self, resource):
        resp = []
        for li in self.client.request('get', [self.class_url(), self.id,
                                              resource.class_url()]):
...
class CreatableObject(APIObject):
    ...
    def create(cls, data, client):
        ...
        instance._refresh_from(
            client.request('post', [cls.class_url()], data=json.dumps(data),
...
class UpdatableObject(APIObject):
    ...
    def save(self):
        self._refresh_from(self.update(
            self.id, self.__dict__, self.client).__dict__)
    ...
    def update(cls, rid, data, client):
        ...
        for k, v in items:
            if k in cls.editable_fields:
                updates[k] = v
        resp = client.request('put', [cls.class_url(), rid],
